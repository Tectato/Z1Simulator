...
func registerPartCell(part : Movable, gridPos : Vector2):
	...
	var posKey = toPosKey(gridPos, layer)
...
func unregisterPart(part : Movable):
	...
	for cell in cells:
		var key = toPosKey(cell, layer)
		occupancy[key].erase(part)
		if occupancy[key].is_empty():
			occupancy.erase(key)
	occupies.erase(part)

func getIntersectionCandidates(pos : Vector3, layer : int):
	var gridPos = toGridPos(pos)
	var key = toPosKey(gridPos, layer)
	var output = []
	if occupancy.has(key):
		output.append_array(occupancy[key])
	key = toPosKey(gridPos, -1) # Check globals as well
	if occupancy.has(key):
		output.append_array(occupancy[key])
...
func toGridPos(pos : Vector3):
	return Vector2(pos.x,pos.z) / Global.workspace.gridSize

func toPosKey(gridPos : Vector2, layer : int):
	return str(layer) + "/" + str(int(gridPos.x)) + "_" + str(int(gridPos.y))
