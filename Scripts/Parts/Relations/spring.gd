...
func applyMove(dir : Vector2, initiator, chain = []):
	...
	if A == initiator:
		...
		if Space.toVec3(globalDir).dot(toInit) > 0 and currentDist >= initialDist \
		or Space.toVec3(globalDir).dot(toInit) < 0 and currentDist <= initialDist:
...
func updateMesh():
	$MeshPivot.look_at(B.global_position + Vector3.UP * 0.02)
