...
func _draw_gizmo() -> void:
	...
	gizmo_spheroid = Gizmo3D.create_spheroid_with_transform(Color.DARK_GREEN, gizmo_size, get_spheroid_transform())
	gizmo_spheroid_outline = Gizmo3D.create_spheroid_outline_with_transform(Color.ORANGE, gizmo_size, get_spheroid_outline_transform())
	_draw_normals()
	Gizmo3D.create_line_transform(Color.RED, Vector3.ZERO, Vector3.UP)
